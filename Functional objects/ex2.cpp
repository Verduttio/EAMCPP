...
int main(){
    ...
    int center = 50;
    std::sort(v.begin(), v.end(), [&center](int a, int b){return abs(center - a) <= abs(center - b);});            // 1
    print(v, "center 50:");
    ...
    int start = 5, step =2;
    auto arithmeticGenerator =  [start, &step, started = false]()mutable {
        if(!started)  {
            started = true;
            return start;
        }
    ...
    auto l1_norm = [](auto& container){
        return std::accumulate(container.begin(), container.end(), 0.0, [](auto a, auto b) {
            return abs(a) + abs(b);
        });
    };                            // [ 5 ]
    ...
    auto polynomial = [](double * a, int n){
        return [a, n](auto point){
            int result = a[0];
            for(int i = 1; i < n+1; i++) {
                result += a[i]*pow(point, i);
            }
            return result;
        };
    };   // ( 6 )
    ...
    // w1(2.0)
    // 1 + 2*2 + 3*4 + 4*8 + 5*16 = 1 + 48 + 80 = 129
